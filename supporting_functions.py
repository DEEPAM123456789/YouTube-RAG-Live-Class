...
def get_important_topics(transcript):
    try:
        prompt = ChatPromptTemplate.from_template("""
        You are an assistant that extracts the 8-9 most important topics discussed in a video transcript or summary.

               Rules:
               - Summarize into exactly 8-9 major points.
               - Each point should represent a key topic or concept, not very small details.
...
def generate_notes(transcript):
    try:
        ...
        You are an AI note-taker. Your task is to read the following YouTube video transcript 
                ...
                - If there are mathematical formulae(with clean format) mention separately about them too at the end, if they are important.
                Here is the transcript:
                {transcript}
    """)
        # Runnable chain
...
# Functions to create embedding and store it into a vector store
